...
func TestShardedCache(t *testing.T) {
	...
	for _, v := range shardedKeys {
		tc.Set(v, "value", DefaultExpiration, NoRefreshDeadline)
	}
}
...
func benchmarkShardedCacheGet(b *testing.B, exp time.Duration) {
	...
	tc.Set("foobarba", "zquux", DefaultExpiration, NoRefreshDeadline)
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		tc.Get("foobarba")
	}
...
func benchmarkShardedCacheGetManyConcurrent(b *testing.B, exp time.Duration) {
	...
	for i := 0; i < n; i++ {
		...
		keys[i] = k
		tsc.Set(k, "bar", DefaultExpiration, NoRefreshDeadline)
	}
	each := b.N / n
